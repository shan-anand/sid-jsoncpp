...
TEST_F(ValueTest, DefaultConstructor)
{
  ...
  EXPECT_TRUE(v.is_basic_type());
  EXPECT_FALSE(v.is_complex_type());
...
TEST_F(ValueTest, NumericOperations)
{
  ...
  // Signed integer
  {
    value v(static_cast<int64_t>(-43));
    v = static_cast<int64_t>(-42);
    EXPECT_FALSE(v.empty());
    EXPECT_TRUE(v.is_num());
    EXPECT_TRUE(v.is_signed());
    EXPECT_TRUE(v.is_decimal());
    EXPECT_TRUE(v.is_basic_type());
    EXPECT_FALSE(v.is_complex_type());
  ...
  // Unsigned integer
  {
    value v(static_cast<uint64_t>(101));
  ...
  // Double
  {
    ...
    EXPECT_TRUE(v.is_num());
    EXPECT_TRUE(v.is_basic_type());
    EXPECT_FALSE(v.is_decimal());
    EXPECT_FALSE(v.is_complex_type());
    EXPECT_DOUBLE_EQ(v.get_double(), 3.14);
    value v1(static_cast<long double>(3.14159));
    v = static_cast<long double>(3.14159);
  }
}
...
TEST_F(ValueTest, ArrayOperations)
{
  ...
  EXPECT_TRUE(arr.is_complex_type());
  EXPECT_FALSE(arr.is_basic_type());
...
TEST_F(ValueTest, ObjectOperations)
{
  ...
  EXPECT_TRUE(obj.is_complex_type());
  EXPECT_FALSE(obj.is_basic_type());
...
TEST_F(ValueTest, TypeConversions)
{
  ...
  EXPECT_THROW(v.has_index(1), std::runtime_error);
  EXPECT_THROW(v.has_key("key1"), std::runtime_error);
  EXPECT_THROW(v.get_keys(), std::runtime_error);
  EXPECT_THROW(v.size(), std::runtime_error);
  EXPECT_THROW(v.get_array(), std::runtime_error);
  EXPECT_THROW(v.get_object(), std::runtime_error);
}
...
TEST_F(ValueTest, EraseOperations)
{
  ...
  EXPECT_NO_THROW(obj.get_object());
  obj["key4"] = nullptr;
  value val;
  EXPECT_EQ(obj.get_value("key1", val), 1);
  EXPECT_EQ(obj.get_value("key4", val), -1);
  EXPECT_EQ(obj.get_value("key10", val), 0);
}
